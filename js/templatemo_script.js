...
function loadTools() {
    //return;
    var template = ' <a href="{url}" class="btn {class}" target="_blank">{name}</a>';
    var htmlContent = '';
    $.get('tools.json', function (res) {
        $.each(res, function (index, section) {
            htmlContent += '<h3>{0}</h3>'.format(section.category);
            //htmlContent += '<p>';
            $.each(section.tools, function (toolIndex, tool) {
                tool.class=typeof (tool.class)==='undefined'?'btn-primary':tool.class;
                htmlContent += formatTemplate(tool, template);
            });
            //htmlContent += '</p>';
        });
        $('#toolsContainer').html(htmlContent);
    })
}
